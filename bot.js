// ==========================================
// 🤖 MOTEUR D'IA ÉVOLUTIVE (V21 - Boucle Auto-Train)
// ==========================================
...
// Fonction pour gérer le bouton classique
function toggleBot() {
...
// NOUVELLE FONCTION : Gère la boucle d'apprentissage automatique
function toggleAutoTrain() {
    ...
    if(autoTrainEnabled) {
        ...
        console.log("🧬 MODE AUTO-TRAIN ACTIVÉ ! L'IA va boucler jusqu'à battre le record.");
        if(!botEnabled) toggleBot(); // Active le bot automatiquement
    } else {
        if(btnTrain) { btnTrain.innerText = "🧬 Auto-Train: OFF"; btnTrain.style.background = "#009688"; }
        console.log("🧬 Mode Auto-Train désactivé.");
    }
}

function triggerBot(botId) {
    if(botTimeout) clearTimeout(botTimeout);
    botTimeout = setTimeout(() => { botThinkAndAct(botId); }, 20); // VITESSE MAXIMALE ! (20ms)
}
...
function botThinkAndAct(botId) {
    try {
        ...
        // --- FIN DE PARTIE ET GESTION DE LA BOUCLE AUTO-TRAIN ---
        if(state.gameOver) {
            ...
            dbBrain.once("value").then(snap => {
                ...
                if(finalScore > currentBest) {
                    ...
                    // On stoppe l'auto-train et le bot pour pouvoir admirer le plateau gagnant !
                    ...
                    // ÉCHEC : MUTATION MAUVAISE
                    console.log(`❌ Échec (Score: ${finalScore}💰 / Record: ${currentBest}💰).`);
                    
                    if(autoTrainEnabled) {
                        console.log("🔄 Auto-Train : Relance automatique d'une nouvelle partie...");
                        // On recrée une nouvelle mutation à partir du MEILLEUR cerveau (et non de l'actuel raté)
                        if(snap.exists()) botDNA = mutateDNA(snap.val().dna);
                        
                        // Relance instantanément la partie (initNewGame est dans index.html)
                        if (typeof initNewGame === "function") {
                            initNewGame(botId, state.mode);
                        }
                    } else {
                        // Arrêt normal si l'Auto-Train n'est pas activé
                        botEnabled = false; 
                        let btnBot = document.getElementById('bot-btn');
                        if(btnBot) { btnBot.innerText = "🤖 Bot: OFF"; btnBot.style.background = "#9c27b0"; }
                    }
        ...
        // ===============================================
        // LA LOGIQUE DE CHOIX RESTE IDENTIQUE (Instinct + ADN)
        // ===============================================
